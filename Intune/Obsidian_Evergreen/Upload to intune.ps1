...
$IntuneWin32AppParameters = @{
	...
	DetectionRule        = New-IntuneWin32AppDetectionRuleFile -Path "%localappdata%\Obsidian" -Existence -FileOrFolder "Obsidian.exe" -DetectionType exists
	Icon 	 	         = New-IntuneWin32AppIcon -FilePath "$psscriptroot\package\AppDeployToolkit\AppDeployToolkitLogo.png" 
	Verbose              = $true
      AppVersion           = "Latest"
	Notes                = "Assign this application to install Obsidian to the user."
...
$IntuneWin32AppParameters.DisplayName               = "Obsidian (Upgrader)" 
