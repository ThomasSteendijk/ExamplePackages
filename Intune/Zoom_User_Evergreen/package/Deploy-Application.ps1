...
Try {
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
	  ...
	  $AppActive = get-process -Name "Zoom"  -ErrorAction SilentlyContinue
	  if ($AppActive) {
	  ...
	  winget install Zoom.Zoom --accept-source-agreements | write-log
	  ...
	  If ($AppActive) {
	  	execute-process -path "$env:APPDATA\Zoom\bin\Zoom.exe"
	  }
    }
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'Zoom' -CloseAppsCountdown 600
	  ...
	  winget uninstall Zoom.Zoom --accept-source-agreements | write-log
